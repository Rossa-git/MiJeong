package com.mijeong.util;

import lombok.Data;

@Data
public class PageVO {

	private int startPage; //첫페이지번호
	private int endPage; //마지막페이지번호
	private boolean next; //다음버튼
	private boolean prev; //이전버튼
	
	private int total; //총게시글수
	private int pageNum; //조회하는 페이지 번호(cri도 존재)
	private int amount; //보여질 데이터 개수
	
	private Criteria cri;
	
	public PageVO(Criteria cri, int total) {
		this.pageNum = cri.getPageNum();
		this.amount = cri.getAmount();
		this.total = total;
		
		this.cri = cri;
		
		//끝페이지
		//(int)Math.ceil( pageNum / 보여질 페이지 수) * 보여질 페이지 수
		this.endPage = (int)Math.ceil( this.pageNum / 10.0 ) * 10;
		
		//시작페이지
		//끝페이지 - 보여질 페이지 수 + 1
		this.startPage = this.endPage - 10 + 1;
		
		//실제 마지막 페이지
		//(int)Math.ceil( 전체 게시글 수/화면에 뿌려질 데이터 amount값)
		int realEnd = (int)Math.ceil( this.total / (double)this.amount );
		
		
		//endPage를 다시 계산
		//404개의 게시물이 있다고 할 때,
		//페이지넘버 31번 -> endPage 40, realEnd 41
		//페이저넘버 41번 -> endPage 50, realEnd 41
		if(this.endPage > realEnd) {
			this.endPage = realEnd; //페이지 마지막에 도달했을 때 보여질 번호 변경
		}
		
		//이전버튼 활성화 여부
		//startPage는 1, 11, 21 ...
		this.prev = this.startPage > 1;
		
		//다음버튼 활성화여부
		this.next = realEnd > this.endPage;
		
	}
	
	
}